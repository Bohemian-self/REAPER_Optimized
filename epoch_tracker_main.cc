...
const char* kHelp = "Usage: <bin> -i <input_file> "
    ...
    "-j number of threads for parallel computation (0=auto, def. 0)\n"
...
int main(int argc, char* argv[]) {
  ...
  while ((opt = getopt(argc, argv, "i:f:p:c:htse:x:m:u:w:aj:d:")) != -1) {
    switch(opt) {
      case 'i':
        filename = optarg;
        break;
...
#ifdef REAPER_USE_OPENMP
  if (num_threads > 0) {
    fprintf(stdout, "Using %d threads for parallel computation\n", num_threads);
  } else {
    fprintf(stdout, "Using %d available threads for parallel computation\n",
            omp_get_max_threads());
  }
#endif
  ...
  et.set_num_threads(num_threads);
